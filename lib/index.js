...
var WINDOWS_GEMINI_ENV = {
  GEMINI_CLI_TRUST_WORKSPACE: "true"
};
var WINDOWS_GEMINI_BAT = "D:\\AI Projects\\GCA\\start_proxy_moto_gemini.bat";
...
function windowsGeminiEnvironment() {
  const environment = { ...process.env };
  const configuredNames = new Set(Object.keys(WINDOWS_GEMINI_ENV).map((name2) => name2.toLowerCase()));
  for (const name2 of Object.keys(environment)) {
    if (configuredNames.has(name2.toLowerCase())) delete environment[name2];
  }
...
async function openProjectTerminal(projectPath) {
  ...
  const command = process.platform === "darwin" ? process.env.SHELL || "/bin/zsh" : process.platform === "win32" ? "cmd.exe" : void 0;
  if (command === void 0) throw new Error("\u5F53\u524D\u7CFB\u7EDF\u6682\u4E0D\u652F\u6301\u9879\u76EE\u7EC8\u7AEF");
  const args = process.platform === "darwin" ? ["-l"] : ["/d", "/q", "/k", `chcp 65001>nul & call "${WINDOWS_GEMINI_BAT}"`];
  ...
  child.onData((value) => {
    appendTerminalOutput(terminal, value);
  });
  child.onExit(({ exitCode }) => {
    terminal.status = "exited";
    appendTerminalOutput(terminal, `\r
[\u7EC8\u7AEF\u5DF2\u9000\u51FA\uFF0C\u4EE3\u7801 ${exitCode}]\r
`);
